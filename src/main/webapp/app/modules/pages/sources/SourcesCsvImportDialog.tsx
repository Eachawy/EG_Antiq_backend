...
interface SourcesCsvImportDialogProps {
  visible: boolean;
  onHide: () => void;
  onImportComplete: () => void;
}

const SourcesCsvImportDialog: React.FC<SourcesCsvImportDialogProps> = ({
  visible,
  onHide,
  onImportComplete,
}) => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${GATEWAY_SERVER_API_URL}/v1/sources/import-csv`,
        {
          method: "POST",
          headers: {
      ...
      if (result.errors === 0) {
        toast.success(`Successfully imported ${result.created} source(s)!`);
      } else {
        toast.warning(
          `Imported ${result.created} source(s) with ${result.errors} error(s)`,
        );
      }
  ...
  const downloadTemplate = () => {
    // Create a sample CSV template for sources
    const csvContent = `titleEn,titleAr,authorEn,authorAr,publicationYear,publisher,sourceType,url,pages,volume,issue,isbn,doi,notes
"The Complete Temples of Ancient Egypt","معابد مصر القديمة","Richard H. Wilkinson","ريتشارد ويلكينسون",2000,"Thames & Hudson",BOOK,"https://example.com",256,,,9780500051009,,"Temple guide"
"Ancient Egyptian Architecture","العمارة المصرية القديمة","Dieter Arnold","ديتر أرنولد",1991,"Oxford University Press",BOOK,"https://example.com",320,,,9780195036336,,"Architecture study"`;
    ...
    link.setAttribute("download", "sources_template.csv");
  ...
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm kemetra-gallery-dialog-subtitle">
              Bulk import academic sources and references from a CSV file
            </p>
          </div>
        </div>
      }
    ...
    >
      <div className="space-y-5 pt-4">
        ...
        <Message
          severity="info"
          text="Required columns: titleEn, titleAr, authorEn, authorAr. Optional: publicationYear, publisher, sourceType, url, pages, volume, issue, isbn, doi, notes"
          className="mb-4"
        />
...
export default SourcesCsvImportDialog;
