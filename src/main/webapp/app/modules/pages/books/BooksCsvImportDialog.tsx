...
interface BooksCsvImportDialogProps {
  visible: boolean;
  onHide: () => void;
  onImportComplete: () => void;
}

const BooksCsvImportDialog: React.FC<BooksCsvImportDialogProps> = ({
  visible,
  onHide,
  onImportComplete,
}) => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${GATEWAY_SERVER_API_URL}/v1/books/import-csv`,
        {
          method: "POST",
          headers: {
      ...
      // Backend returns result directly, not wrapped in data object
      setImportResult(result);

      if (result.errors === 0) {
        toast.success(`Successfully imported ${result.created} book(s)!`);
      } else {
        toast.warning(
          `Imported ${result.created} book(s) with ${result.errors} error(s)`,
        );
      }
  ...
  const downloadTemplate = () => {
    // Create a sample CSV template for books
    const csvContent = `titleEn,titleAr,authorEn,authorAr,coverImage,publicationYear,publisher,isbn,pages,description,readUrl,purchaseUrl,language,edition
"Ancient Egyptian Architecture","العمارة المصرية القديمة","Dieter Arnold","ديتر أرنولد","uploads/books/arch.jpg",1991,"Oxford University Press",9780195036336,320,"Comprehensive study of ancient Egyptian architecture","https://archive.org/details/example","https://amazon.com/example","en","1st"
"The Oxford History of Ancient Egypt","تاريخ أكسفورد لمصر القديمة","Ian Shaw","إيان شو","uploads/books/oxford.jpg",2000,"Oxford University Press",9780192804587,512,"Authoritative history of ancient Egypt","https://archive.org/details/example2","https://amazon.com/example2","en","1st"`;
  ...
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm kemetra-gallery-dialog-subtitle">
              Bulk import related books and publications from a CSV file
            </p>
          </div>
        </div>
      }
    ...
    >
      <div className="space-y-5 pt-4">
        ...
        <Message
          severity="info"
          text="Required columns: titleEn, titleAr, authorEn, authorAr. Optional: coverImage, publicationYear, publisher, isbn, pages, description, readUrl, purchaseUrl, language, edition"
          className="mb-4"
        />
...
export default BooksCsvImportDialog;
